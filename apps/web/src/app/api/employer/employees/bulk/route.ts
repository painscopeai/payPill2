...
export async function POST(request: NextRequest) {
	...
	for (const row of parsed.rows) {
		...
		if (hire_raw) {
			...
			if (Number.isFinite(d.getTime())) hire_date = hire_raw.slice(0, 10);
		}
		...
		if (banErr) {
			failures.push({ rowNumber, message: banErr.message || 'Failed to restrict login until approval' });
		...
		if (pendErr) {
			if (isPendingPasswordTableUnavailableError(pendErr)) {
				if (!loggedMissingPendingPasswordTable) {
					...
					console.warn(
						'[employer/bulk] employer_employee_pending_passwords missing; approval flow will not offer password copy until migrated.',
					);
				}
				successCount++;
				continue;
