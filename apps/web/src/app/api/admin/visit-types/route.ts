...
import { createVisitType, listVisitTypes } from '@/server/admin/appointmentReferenceService';
...
export async function GET(request: NextRequest) {
	...
	try {
		const items = await listVisitTypes(includeInactive);
...
export async function POST(request: NextRequest) {
	...
	try {
		const row = await createVisitType({
			slug: body.slug,
			label: body.label,
