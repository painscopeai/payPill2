...
import { createProviderType } from '@/server/admin/providerTypeService';
...
export async function POST(request: NextRequest) {
	...
	const hv = validateHeadersForKind('provider_types', parsed.headers);
	if (!hv.ok) {
	...
	for (const row of parsed.rows) {
		...
		try {
			await createProviderType({ slug, label, sort_order, active });
			successCount++;
		} catch (e: unknown) {
	...
	await auditLog({
		adminId: ctx.adminId,
		action: 'BULK_IMPORT_PROVIDER_TYPES',
		resourceType: 'provider_types',
		resourceId: null,
