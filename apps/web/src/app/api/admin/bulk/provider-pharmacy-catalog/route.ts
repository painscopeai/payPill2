...
export async function POST(request: NextRequest) {
	...
	const hv = validateHeadersForKind('provider_pharmacy_catalog', parsed.headers);
	if (!hv.ok) {
	...
	for (const row of parsed.rows) {
		...
		const unit_price = Number.parseFloat(String(row.unit_price ?? '0'));
		if (!Number.isFinite(unit_price) || unit_price < 0) {
			failures.push({ rowNumber, message: 'Invalid unit_price' });
			continue;
		}
		const qoh = Number.parseInt(String(row.quantity_on_hand ?? '0'), 10);
		if (!Number.isFinite(qoh) || qoh < 0) {
			failures.push({ rowNumber, message: 'Invalid quantity_on_hand' });
			continue;
		}
		const lowRaw = String(row.low_stock_threshold ?? '').trim();
		const low_stock_threshold =
			lowRaw === '' ? null : Number.parseInt(lowRaw, 10);
		if (low_stock_threshold != null && (!Number.isFinite(low_stock_threshold) || low_stock_threshold < 0)) {
			failures.push({ rowNumber, message: 'Invalid low_stock_threshold' });
			continue;
		}
		const currency = String(row.currency ?? '').trim().toUpperCase().slice(0, 8) || 'USD';
		const default_strength = String(row.default_strength ?? '').trim().slice(0, 200) || null;
		const default_route = String(row.default_route ?? '').trim().slice(0, 100) || null;
		const default_frequency = String(row.default_frequency ?? '').trim().slice(0, 200) || null;
		...
		const { error: insErr } = await sb.from('provider_drug_catalog').insert({
	...
	await auditLog({
		adminId: ctx.adminId,
		action: 'BULK_IMPORT_PROVIDER_PHARMACY_CATALOG',
		resourceType: 'provider_drug_catalog',
