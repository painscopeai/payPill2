...
import { createVisitType } from '@/server/admin/appointmentReferenceService';
...
export async function POST(request: NextRequest) {
	...
	const hv = validateHeadersForKind('visit_types', parsed.headers);
	if (!hv.ok) {
	...
	for (const row of parsed.rows) {
		...
		try {
			await createVisitType({ slug, label, sort_order, active });
			successCount++;
		} catch (e: unknown) {
	...
	await auditLog({
		adminId: ctx.adminId,
		action: 'BULK_IMPORT_VISIT_TYPES',
		resourceType: 'visit_types',
		resourceId: null,
