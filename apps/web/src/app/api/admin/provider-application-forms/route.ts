...
/**
 * Published provider_application forms for onboarding (invite links).
 * Uses manage_providers so Provider Onboarding works without manage_forms.
 */
export async function GET(_request: NextRequest) {
	...
	try {
		const { data, error } = await sb
			...
			.eq('form_type', 'provider_application')
			.eq('status', 'published')
