...
import { createInsuranceOption, listInsuranceOptions } from '@/server/admin/appointmentReferenceService';
...
export async function GET(request: NextRequest) {
	...
	try {
		const items = await listInsuranceOptions(includeInactive);
...
export async function POST(request: NextRequest) {
	...
	try {
		const row = await createInsuranceOption({
			slug: body.slug,
			label: body.label,
			sort_order: typeof body.sort_order === 'number' ? body.sort_order : undefined,
			active: body.active,
			copay_estimate:
				body.copay_estimate === null || body.copay_estimate === undefined
					? undefined
		...
		await auditLog({
			adminId: ctx.adminId,
			action: 'CREATE_INSURANCE_OPTION',
