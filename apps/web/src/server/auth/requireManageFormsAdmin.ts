...
export async function requireManageFormsAdmin(
	request: NextRequest,
): Promise<ManageFormsContext | NextResponse> {
	...
	if (!perms.includes('manage_forms') && !perms.includes('*')) {
		return NextResponse.json({ error: 'Insufficient permissions: manage_forms required' }, { status: 403 });
	}
	return {
		adminId: uid,
